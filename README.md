...
- 2 進数と n 進数
  ...
  - 8 進数なら 0~8 という数字を使って表現し、8 倍ずつ桁があがっていく
  - 16 進数なら 0 ~ 9 の数字と A ~ F のアルファベット使って表示し、16 倍ずつ桁が上がっていく
  - n にあたる数字のことを基数と呼びます
...
- **共通鍵（秘密鍵）暗号方式**：暗号化と復号に同じ鍵を利用する暗号方式。
  処理時間がかからないメリットがありますが、暗号化した鍵を受信者に送る過程で
  第三者に盗聴されるリスクがあります。
- **公開鍵暗号方式とディジタル署名**：暗号化と復号に異なる鍵を利用する暗号方式。１つを公開し（公開鍵）、もう一方を非公開に（秘密鍵）する。公開鍵は不特定多数の人が入手できるようになっていますが、秘密鍵は自分自信のみ持っています。秘密鍵で暗号化したものは、公開鍵でしたか復号はできず、またその逆もしかりです。
  これを応用した技術にメールダイジェストを使うディジタル署名があります。

- **メッセージダイジェスト、ハッシュ関数**：ディジタル署名で、通信の改ざんが検知としてつかわれるのがメッセージダイジェストです。メッセージダイジェストは、通信文からハッシュ関数とよばれる不可逆性のある関数から生成され、元に戻すことはできません。この性質を利用して、送信者が生成したメッセージダイジェストと、正当な受信者が受け取った通信文からハッシュ関数で生成したものが一致すれば、改ざんはないことを証明できます。

- **DMZ（Demilitarized Zone）**：非武装地帯と約され、インターネットに接続されたネットワークでファイアウォールによって、外部、内部のネットワークからも隔離された区域を意味する。外部に公開されるサーバはファイアウォールによって外部からの不正なアクセスを排除できます。
